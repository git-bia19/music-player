...
const Playlist = (_ => {
  ...
  const playerImageEl = document.querySelector(".player__thumbnail");
  const playerTitleEl = document.querySelector(".player__details");
 ...
 const changeImage = _ =>{
  currentSong.src =  songs[currentlyPlayingIndex].image;
  let imageMarkup = `<img src=${currentSong.src} alt="" class="player__img">`
  playerImageEl.innerHTML = imageMarkup
 }
    ...
    songs.forEach((songObj, index) => {
      ...
      markup += `
        <li class="playlist__song ${index === currentlyPlayingIndex ? 'playlist__song--active' : ''}">
          ...
          <img src=${songObj.image} alt="" class="sideplayer__img">
        </li>
      `
